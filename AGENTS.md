...
- New PRs must not add complexity to production/runtime code; tests and test harnesses are exempt. If runtime complexity is unavoidable, the PR must categorically justify why the task or problem cannot be solved without it and why simpler alternatives are insufficient.
