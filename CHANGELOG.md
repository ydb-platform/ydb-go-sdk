* `database/sql` driver (no public API changes): `QueryContext` / `Stmt.QueryContext` / `Tx.QueryContext` cancellation now propagates through row iteration consistently—the SQL `Rows` adapter retains that context for every `Next`, `NextResultSet`, and column-metadata call, and both Query Service and Table Service backends receive it where supported.
* When using **QueryService** (`ydb.WithQueryService(true)`, default), canceling the query context after starting to read a result set surfaces `context.Canceled` from `(*sql.Rows).Err()` after `Next` stops, matching typical `database/sql` expectations for context-aware queries.
* When using **TableService** (`ydb.WithQueryService(false)`), row iteration still relies on the table client read path; `Rows.Err()` after cancel may be empty or `io.EOF` once rows are exhausted, rather than the cancellation error.
