* Added `ydb.BeginTransaction` (CLIENT) span emitted by the `spans` package for the explicit `QueryService.BeginTransaction` RPC (eager `s.Begin` and `Tx.UnLazy`). Lazy transactions started via `DoTx` continue to fuse the begin into the first `ExecuteQuery` and do not produce this span
...
* Renamed the cluster-discovery span emitted by the `spans` package to `ydb.Driver.Initialize` (INTERNAL) and limited it to the very first driver initialization; periodic discovery refreshes from the driver's repeater no longer produce a span
* Renamed the pool-get span emitted by the `spans` package to `ydb.GetSession` (INTERNAL); on a pool miss it parents a child `ydb.CreateSession`
* Suppressed noisy internal-package spans previously emitted by the `spans` package (`internal/query.(*Client).DoTx`, `internal/query.(*Client).Do`, `internal/pool.(*Pool).With`, `internal/pool.(*Pool).try`, `internal/pool.(*Pool).putItem`, `internal/query.(*Session).Begin`, `internal/conn.(*conn).Invoke`, `internal/conn.(*conn).dial`, `internal/conn.(*conn).NewStream`, session lifecycle deletes); the resulting span tree only contains the user-facing `ydb.*` names listed in the spec
* Wired `query.Client.Do` and `query.Client.DoTx` to propagate the driver-level `trace.Retry` (e.g. installed via `spans.WithTraces`) into their internal retry loop, so `ydb.RunWithRetry` / `ydb.Try` spans now appear for query-service operations
* The `spans` adapter attaches `network.peer.address`, `network.peer.port` and `ydb.node.dc` to the surrounding client span (e.g. `ydb.ExecuteQuery`, `ydb.Commit`) once the gRPC layer selects a peer endpoint, instead of producing a separate `internal/conn.(*conn).Invoke` child span. The W3C `traceparent` header continues to be propagated to the server
* Added `spans.Span.SetAttributes` to the experimental `spans` interface so adapters can attach late-bound attributes (such as `network.peer.*`) to a span without ending it
