...
func TestSanitizeDSN(t *testing.T) {
	// Simple test to verify the wrapper delegates correctly to dsn.SanitizeDSN
	// Comprehensive testing is done in internal/dsn/dsn_test.go
	result := sanitizeDSN("grpc://user:password@localhost:2135/local")
	require.Equal(t, "grpc://user:%2A%2A%2A@localhost:2135/local", result)
}
