...
func NewStorage(_ context.Context, fw *framework.Framework) (framework.Workload, error) {
	params := kv.ParseParams(fw, "gorm-table", nil)
...
func (d *db) Read(ctx context.Context, id generator.RowID) (_ generator.Row, attempts int, err error) {
	...
	var r generator.Row
	err = retry.Do(ydbSDK.WithTxControl(ctx, readTx), rawDB,
...
func (d *db) Write(ctx context.Context, row generator.Row) (attempts int, err error) {
	...
	err = retry.Do(ydbSDK.WithTxControl(ctx, writeTx), rawDB,
		func(ctx context.Context, _ *sql.Conn) error {
			if err = ctx.Err(); err != nil {
				return err
			}
...
func (d *db) CreateTable(ctx context.Context) error {
	...
	return d.gormDB.WithContext(ctx).Scopes(scopeTable(d.tableName)).
		Set("gorm:table_options", d.tableOptions).AutoMigrate(&generator.Row{})
}
...
func (d *db) DropTable(ctx context.Context) error {
	...
	return d.gormDB.WithContext(ctx).Scopes(scopeTable(d.tableName)).Migrator().DropTable(&generator.Row{})
}
