...
func runTopicMultiWriterPerformanceWithPartitionIDVsManualWriters(t *testing.T, waitServerAck bool) {
	...
	topicPath := fmt.Sprintf("%s/%s-%s", scope.Folder(), t.Name(), uuid.NewString())
	err := topicClient.Create(ctx, topicPath)
	if err != nil {
		if strings.Contains(err.Error(), "Unable to construct channel binding for PQ with the storage pool") {
			t.Skipf("topic storage pool is not configured in this environment: %v", err)
		}
		require.NoError(t, err)
	}
	t.Cleanup(func() {
	...
	t.Run("manual_writer_selection_by_partition_id", func(t *testing.T) {
		...
		for i := range messageCount {
			partitionID := partitionIDs[i%len(partitionIDs)]
			require.NoError(t, writers[partitionID].Write(ctx, topicwriter.Message{
				Data: bytes.NewReader(payload),
			}))
		}
