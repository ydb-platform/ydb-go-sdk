Feature: Partition split with a pending transaction and complete replay
  ...
  # AlterTopic makes partition 0 inactive with active children [1,2]. The old stream
  # still ACKs seq_no=2 as written_in_tx after split, but commit is ABORTED
  # (#2011, partition 0 inactive). All partition end offsets remain 0.
  # A new transaction replays both logical payloads through producerless child streams
  # and commits successfully. Each child ends at offset 1; StreamRead returns
  # staged-before-split and sent-after-split once each.
  ...
  # Observed on YDB main.7f40cb4 (trunk, 2026-09-08), without producer_id:
  # the parent ACK is recorded but withheld and its stream is cancelled. AlterTopic
  # makes parent 0 inactive with children [1,2]; explicit rollback succeeds.
  # A new transaction through producerless child streams commits both replayed payloads:
  # parent end_offset=0, each child end_offset=1. StreamRead returns logical-left and
  # logical-right once each. This is replay after a known rollback, not deduplication.
