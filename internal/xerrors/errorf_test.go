...
func TestErrorf(t *testing.T) {
	for _, tt := range []struct {
		...
		{
			err: Errorf("format int(%d) and string(%q)", 100500, "azaza"),
			s:   "format int(100500) and string(\"azaza\")",
		},
		{
			err: Errorf("format int(%d) and string(%q) (error '%w')", 100500, "azaza", context.Canceled),
			iss: []error{context.Canceled},
			ass: nil,
			s:   "format int(100500) and string(\"azaza\") (error 'context canceled')",
		},
		{
			err: Errorf("format int(%d), error '%w' and string(%q)", 100500, context.Canceled, "azaza"),
			iss: []error{context.Canceled},
			ass: nil,
			s:   "format int(100500), error 'context canceled' and string(\"azaza\")",
		},
		{
			err: Errorf("format int(%d), error1 '%w', error2 '%w' and string(%q)",
				100500, context.Canceled, context.DeadlineExceeded, "azaza",
			),
		...
		{
			err: Errorf("format int(%d), error1 '%w', error2 '%w' and string(%q)",
				100500, Wrap(context.Canceled), context.DeadlineExceeded, "azaza",
			),
			iss: []error{context.Canceled, context.DeadlineExceeded},
