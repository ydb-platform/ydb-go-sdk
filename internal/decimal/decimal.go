...
func Parse(s string, precision, scale uint32) (*big.Int, error) {
	...
	var dot bool
	bInt, done, err := loopOverEachCharacter(s, dot, scale, v, integral, neg)
	if done {
		return bInt, err
	}
	//nolint:nestif
...
// loopOverEachCharacter iterates over each character in the string and performs certain actions based on the
// character's value and position.
func loopOverEachCharacter(
	s string,
	dot bool,
	scale uint32,
	v *big.Int,
