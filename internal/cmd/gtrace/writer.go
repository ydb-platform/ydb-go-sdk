...
func (w *Writer) Write(p Package) error {
	w.pkg = p.Package
	w.deprecated = p.DeprecatedFields
...
func (w *Writer) funcImports(dst []dep, fn *Func) []dep {
	for i := range fn.Params {
		...
		n, s := unwrapStruct(fn.Params[i].Type)
		if s != nil {
			typeName := ""
			if n != nil {
				typeName = n.Obj().Name()
			...
			forEachField(s, func(v *types.Var) {
				if !v.Exported() || w.fieldDeprecated(typeName, v.Name()) {
					return
				}
...
func (w *Writer) flattenParams(params []Param) (dst []Param) {
	for i := range params {
		n, s := unwrapStruct(params[i].Type)
		if s != nil {
			dst = w.flattenStruct(dst, n, s)
...
func (w *Writer) flattenStruct(dst []Param, n *types.Named, s *types.Struct) []Param {
	typeName := ""
	if n != nil {
		typeName = n.Obj().Name()
	}
	forEachField(s, func(f *types.Var) {
		...
		if w.fieldDeprecated(typeName, f.Name()) {
			return
		}
		var (
			name = f.Name()
...
func (w *Writer) fieldDeprecated(typeName, fieldName string) bool {
	if w.deprecated == nil || typeName == "" {
		return false
	}
	fields, ok := w.deprecated[typeName]
	if !ok {
		return false
	}
...
func (w *Writer) constructStruct(n types.Type, s *types.Struct, vars []string) (string, []string) {
	...
	for v := range s.Fields() {
		...
		if w.fieldDeprecated(typeName, v.Name()) {
			continue
		}
		name := vars[0]
		vars = vars[1:]
...
func (w *Writer) hookFuncShortcut(fn *Func, name string) {
	w.newScope(func() {
		w.code(`func(`)
		var (
			params = w.flattenParams(fn.Params)
			names  = w.declareParams(params)
		)
		for i := range params {
			if i > 0 {
