...
func New(ctx context.Context, address string, t trace.Driver) (_ net.Conn, err error) {
	...
	if err != nil {
		return nil, errors.Errorf(0, "New(%v): %w", address, err)
	}
	return &conn{
		address: address,
		cc:      cc,
...
func (c *conn) Read(b []byte) (n int, err error) {
	...
	n, err = c.cc.Read(b)
	if err != nil {
		return n, errors.Errorf(0, "conn.Read(...): %w", err)
	}
	return n, nil
}
...
func (c *conn) Write(b []byte) (n int, err error) {
	...
	n, err = c.cc.Write(b)
	if err != nil {
		return n, errors.Errorf(0, "conn.Write(...): %w", err)
	}
	return n, nil
}
...
func (c *conn) Close() (err error) {
	...
	err = c.cc.Close()
	if err != nil {
		return errors.Errorf(0, "conn.Close(): %w", err)
	}
	return nil
}
