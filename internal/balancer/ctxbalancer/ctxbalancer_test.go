...
func TestCtxBalancer_Create(t *testing.T) {
	b := Balancer([]conn.Conn{&mock.ConnMock{AddrField: "a1", NodeIdField: 1}, &mock.ConnMock{AddrField: "a2", NodeIdField: 2}}).(*ctxBalancer)
	...
	b1 := b.Create([]conn.Conn{&mock.ConnMock{AddrField: "a3", NodeIdField: 3}, &mock.ConnMock{AddrField: "a4", NodeIdField: 4}, &mock.ConnMock{AddrField: "a5", NodeIdField: 5}}).(*ctxBalancer)
	checkOriginalBalancer()
...
func TestCtxBalancer_Next(t *testing.T) {
	b := Balancer([]conn.Conn{&mock.ConnMock{NodeIdField: 1, State: conn.Online}, &mock.ConnMock{NodeIdField: 2, State: conn.Banned}}).(*ctxBalancer)
