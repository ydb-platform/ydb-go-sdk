...
func ExampleAllowImplicitSessions() {
	ctx := context.TODO()

	implicitSessionOpt := ydb.WithQueryConfigOption(query.AllowImplicitSessions())

	db, err := ydb.Open(ctx, "grpc://localhost:2136/local", implicitSessionOpt)
	if err != nil {
		panic(err)
	}
	...
	// Executes without sending `CreateSession` and `AttachSession` requests
	res, err := db.Query().Query(ctx, `SELECT 42 as id, "my string" as myStr`)
	if err != nil {
		panic(err)
	}
	defer res.Close(ctx)
